...
namespace eegs_back_end.Admin.ProfilingSetup.Repository
{
    public interface IProfilingRepository :IGlobalInterface
    {
        bool Insert(ProfilingModel model);
        ...
        bool Edit(string GUID, ProfilingModel model);
        bool Delete(string GUID);
        object GetProfiling(string Profiling_guid);
    }
    public class ProfilingRepository : FormNumberGenerator, IProfilingRepository
    {
        public bool Edit(string GUID, ProfilingModel model)
        {
        ...
        {
            string sql = "UPDATE mswd.Profiling SET status = 'Deleted' where mswd.Profiling.Profiling_guid = @Profiling_guid";
        ...
        {
            string sql = @"SELECT mswd.children_profiling.profiling_guid, mswd.children_profiling.form_trans_no, mswd.children_profiling.application_date, mswd.children_profiling.status
                            FROM mswd.children_profiling
                            WHERE mswd.children_profiling.`status` = 'Active' ORDER BY mswd.children_profiling.id DESC;";

            List<object> obj = (List<object>)QueryModule.DataSource<object>(sql);

            ExpandoObject form = Forms.getForm("profiling");
        ...
        public object GetProfiling(string child_info_guid)
        {
            ...
            var obj = (ProfilingModel)QueryModule.DataObject<ProfilingModel>(sql, new { child_info_guid = child_info_guid });
            ...
            obj.family_details = (List<ProfilingDetails>)QueryModule.DataSource<ProfilingDetails>(sql);
        ...
        public object GetPersonAdd(string GUID)
        {
            ...
            var obj = (ProfilingModel)QueryModule.DataObject<ProfilingModel>(sql);
            ...
            //obj.details = (List<ProfilingDetails>)QueryModule.DataSource<ProfilingDetails>(sql);
        ...
        public bool Insert(ProfilingModel model)
        {
            if (QueryModule.connection.State == System.Data.ConnectionState.Open)
            {
                QueryModule.connection.Close();
