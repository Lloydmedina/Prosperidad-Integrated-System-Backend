using eegs_back_end.Admin.ChildInformationSetup.Model;
...
namespace eegs_back_end.Admin.ChildInformationSetup.Repository
{
    public interface IChildInformationRepository :IGlobalInterface
    {
        bool Insert(ChildInformationModel model);
        ...
        bool Edit(string GUID, ChildInformationModel model);
        bool Delete(string GUID);
        object GetChildInformation(string ChildInformation_guid);
    }
    public class ChildInformationRepository : FormNumberGenerator, IChildInformationRepository
    {
        public bool Edit(string GUID, ChildInformationModel model)
        {
        ...
        {
            string sql = "UPDATE mswd.ChildInformation SET status = 'Deleted' where mswd.ChildInformation.ChildInformation_guid = @ChildInformation_guid";
        ...
        {
            string sql = @"SELECT mswd.child_info.child_info_guid, mswd.child_info.person_guid, general.person.first_name, general.person.middle_name, general.person.last_name, general.person.prefix, 
                            ...
                            mswd.child_info.form_trans_no, general.person.birth_date, general.person.civil_status_id, general.civil_status.civil_status_name, general.person.gender_id, 
                            general.gender.gender_name, mswd.child_info.application_date, mswd.child_info.member_count, mswd.child_info.status
                            FROM mswd.child_info
                            INNER JOIN general.person ON general.person.person_guid = mswd.child_info.person_guid
                            ...
                            WHERE mswd.child_info.status = 'Active' ORDER BY mswd.child_info.id DESC";

            List<object> obj = (List<object>)QueryModule.DataSource<object>(sql);

            ExpandoObject form = Forms.getForm("child-info-sheet");
        ...
        public object GetChildInformation(string child_info_guid)
        {
            ...
            var obj = (ChildInformationModel)QueryModule.DataObject<ChildInformationModel>(sql, new { child_info_guid = child_info_guid });
            ...
            obj.family_details = (List<ChildInformationDetails>)QueryModule.DataSource<ChildInformationDetails>(sql);
        ...
        public object GetPersonAdd(string GUID)
        {
            ...
            var obj = (ChildInformationModel)QueryModule.DataObject<ChildInformationModel>(sql);
            ...
            //obj.details = (List<ChildInformationDetails>)QueryModule.DataSource<ChildInformationDetails>(sql);
        ...
        public bool Insert(ChildInformationModel model)
        {
            if (QueryModule.connection.State == System.Data.ConnectionState.Open)
            {
                QueryModule.connection.Close();
