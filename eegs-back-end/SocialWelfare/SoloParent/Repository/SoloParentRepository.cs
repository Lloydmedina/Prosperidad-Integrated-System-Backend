...
namespace eegs_back_end.Admin.SoloParentSetup.Repository
{
    public interface ISoloParentRepository :IGlobalInterface
    {
        bool Insert(SoloParentModel model);
        ...
        bool Edit(string GUID, SoloParentModel model);
        bool Delete(string GUID);
        object GetSoloParent(string SoloParent_guid);
    }
    public class SoloParentRepository : FormNumberGenerator, ISoloParentRepository
    {
        public bool Edit(string GUID, SoloParentModel model)
        {
            ...
            using (var transaction = QueryModule.connection.BeginTransaction())
            {
                try
                {
                    ...
                    sql = "INSERT INTO mswd.solo_parent_logs (`main_guid`, `user_id`) VALUES('" + model.solo_parent_guid + "', '" + GlobalObject.user_id + "')";
                    QueryModule.Execute<int>(sql, model);
                    ...
                    if (model.family_details.Count > 0)
                    {
                        ...
                        {
                            dt.main_guid = model.solo_parent_guid;
        ...
        {
            string sql = "UPDATE mswd.SoloParent SET status = 'Deleted' where mswd.SoloParent.SoloParent_guid = @SoloParent_guid";
        ...
        public List<object> GetList()
        {
            string sql = @"SELECT mswd.solo_parent.solo_parent_guid, mswd.solo_parent.person_guid, general.person.first_name, general.person.middle_name, general.person.last_name, general.person.prefix, 
                            ...
                            general.gender.gender_name, mswd.solo_parent.application_date, mswd.solo_parent.member_count, mswd.solo_parent.status, general.person.full_name,
                            (SELECT COUNT(*) FROM mswd.solo_parent WHERE mswd.solo_parent.status = 'Active') as 'count'
                            ...
                            WHERE mswd.solo_parent.status = 'Active' ORDER BY mswd.solo_parent.id DESC";
            ...
            sql = @"SELECT z.q as val, z.tag  FROM (
                    ...
                    FROM mswd.solo_parent sp
                    INNER JOIN general.person ON general.person.person_guid = sp.person_guid
                    ...
                    FROM mswd.solo_parent sp
                    LEFT JOIN general.person ON general.person.person_guid = sp.person_guid
                    LEFT JOIN general_address.lgu_brgy_setup_temp ON general_address.lgu_brgy_setup_temp.brgy_id = general.person.barangay_id
                    ) z";
        ...
        public List<object> GetAllList()
        {
            string sql = @"SELECT mswd.solo_parent.solo_parent_guid, mswd.solo_parent.person_guid, general.person.first_name, general.person.middle_name, general.person.last_name, general.person.prefix, 
                            ...
                            mswd.solo_parent.form_trans_no, general.person.birth_date, general.person.civil_status_id, general.civil_status.civil_status_name, general.person.gender_id, 
                            general.gender.gender_name, mswd.solo_parent.application_date, mswd.solo_parent.member_count, mswd.solo_parent.status, general.person.full_name,
                            (SELECT COUNT(*) FROM mswd.solo_parent WHERE mswd.solo_parent.status = 'Active' OR mswd.solo_parent.status = 'Deleted') as 'count'
                            FROM mswd.solo_parent
                            LEFT JOIN general.person ON general.person.person_guid = mswd.solo_parent.person_guid
                            LEFT JOIN general_address.lgu_province_setup_temp ON general_address.lgu_province_setup_temp.province_id = general.person.province_id
                            LEFT JOIN general_address.lgu_city_mun_setup_temp ON general_address.lgu_city_mun_setup_temp.city_mun_id = general.person.citmun_id
        ...
        public List<object> GetListDeleted()
        {
            ...
            string sql = @"SELECT mswd.solo_parent.solo_parent_guid, mswd.solo_parent.person_guid, general.person.first_name, general.person.middle_name, general.person.last_name, general.person.prefix, 
                            ...
                            mswd.solo_parent.form_trans_no, general.person.birth_date, general.person.civil_status_id, general.civil_status.civil_status_name, general.person.gender_id, 
                            general.gender.gender_name, mswd.solo_parent.application_date, mswd.solo_parent.member_count, mswd.solo_parent.status, general.person.full_name,
                            (SELECT COUNT(*) FROM mswd.solo_parent WHERE mswd.solo_parent.status = 'Deleted') as 'count'
                            FROM mswd.solo_parent
                            LEFT JOIN general.person ON general.person.person_guid = mswd.solo_parent.person_guid
                            LEFT JOIN general_address.lgu_province_setup_temp ON general_address.lgu_province_setup_temp.province_id = general.person.province_id
                            LEFT JOIN general_address.lgu_city_mun_setup_temp ON general_address.lgu_city_mun_setup_temp.city_mun_id = general.person.citmun_id
                            ...
                            LEFT JOIN general.civil_status ON general.civil_status.civil_status_id = general.person.civil_status_id
                            LEFT JOIN general.gender ON general.gender.gender_id = general.person.gender_id
                            WHERE mswd.solo_parent.status = 'Deleted' ORDER BY mswd.solo_parent.id DESC";
        ...
        public List<object> GetListGenerated(int? filter_type_status_id = 0, int? status_id = 0, int? status_deleted_id = 0, string? this_month = "", string? this_year = "", string? monthly = "", string? monthlyYear = "", string? year_quarterly = "", int? quarter = 0, string? yearly = "", string? from = "", string? to = "")
        {
            ...
            if (status_id != 0)
            {
                filterStatus = "mswd.solo_parent.status = 'Active'";
            }
            else if (status_deleted_id != 0)
            {
                filterStatus = "mswd.solo_parent.status = 'Deleted'";
            }

            if (filter_type_status_id == 1)
            {
                dateFilter = "AND MONTH(mswd.solo_parent.application_date) = '" + this_month + "'";
            }
            else if (filter_type_status_id == 2)
            {
                dateFilter = "AND YEAR(mswd.solo_parent.application_date) = '" + this_year + "'";
            }
            else if (filter_type_status_id == 3)
            {
                dateFilter = "AND MONTH(mswd.solo_parent.application_date) = '" + monthly + "' AND YEAR(mswd.solo_parent.application_date) = '" + monthlyYear + "'";
            }
            else if (filter_type_status_id == 4)
            {
                dateFilter = "AND YEAR(mswd.solo_parent.application_date) = '" + year_quarterly + "' AND QUARTER(mswd.solo_parent.application_date) = '" + quarter + "'";
            }
            else if (filter_type_status_id == 5)
            {
                dateFilter = "AND YEAR(mswd.solo_parent.application_date) = '" + yearly + "'";
            }
            else if (filter_type_status_id == 6)
            {
                dateFilter = "AND mswd.solo_parent.application_date >= '" + from + "' AND mswd.solo_parent.application_date <= '" + to + "'";
            }
            ...
            string sql = @"SELECT mswd.solo_parent.solo_parent_guid, mswd.solo_parent.person_guid, general.person.first_name, general.person.middle_name, general.person.last_name, general.person.prefix, 
                            ...
                            mswd.solo_parent.form_trans_no, general.person.birth_date, general.person.civil_status_id, general.civil_status.civil_status_name, general.person.gender_id, 
                            general.gender.gender_name, mswd.solo_parent.application_date, mswd.solo_parent.member_count, mswd.solo_parent.status, general.person.full_name,
                            (SELECT COUNT(*) FROM mswd.solo_parent WHERE " + filterStatus + @" " + dateFilter  + @") as 'count'
                            FROM mswd.solo_parent
                            LEFT JOIN general.person ON general.person.person_guid = mswd.solo_parent.person_guid
                            LEFT JOIN general_address.lgu_province_setup_temp ON general_address.lgu_province_setup_temp.province_id = general.person.province_id
                            LEFT JOIN general_address.lgu_city_mun_setup_temp ON general_address.lgu_city_mun_setup_temp.city_mun_id = general.person.citmun_id
                            ...
                            LEFT JOIN general.civil_status ON general.civil_status.civil_status_id = general.person.civil_status_id
                            LEFT JOIN general.gender ON general.gender.gender_id = general.person.gender_id
                            WHERE " + filterStatus + @" " + dateFilter  + @" ORDER BY mswd.solo_parent.id DESC";
        ...
        public object GetSoloParent(string solo_parent_guid)
        {
            ...
            var obj = (SoloParentModel)QueryModule.DataObject<SoloParentModel>(sql, new { solo_parent_guid = solo_parent_guid });
            ...
            obj.family_details = (List<SoloParentDetails>)QueryModule.DataSource<SoloParentDetails>(sql);
        ...
        public object GetPersonAdd(string GUID)
        {
            ...
            var obj = (SoloParentModel)QueryModule.DataObject<SoloParentModel>(sql);
            ...
            //obj.details = (List<SoloParentDetails>)QueryModule.DataSource<SoloParentDetails>(sql);
        ...
        public List<object> GetHistoryLogs(string GUID)
        {
            string sql = @"SELECT mswd.solo_parent_logs.main_guid, mswd.solo_parent.application_date, mswd.solo_parent_logs.user_id, mswd.solo_parent.form_trans_no,
                            ...
                            LEFT JOIN mswd.solo_parent ON mswd.solo_parent.solo_parent_guid = mswd.solo_parent_logs.main_guid  
                            LEFT JOIN general.person ON general.person.person_guid = mswd.solo_parent.person_guid
        ...
        public bool Insert(SoloParentModel model)
        {
            if (QueryModule.connection.State == System.Data.ConnectionState.Open)
            {
                QueryModule.connection.Close();
            ...
            using (var transaction = QueryModule.connection.BeginTransaction())
            {
                try
                {
                    ...
                    model.solo_parent_guid = Guid.NewGuid().ToString();
                    model.status = "Active";
