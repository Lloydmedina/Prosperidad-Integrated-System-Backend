...
namespace eegs_back_end.Admin.ServiceProviderSetup.Repository
{
    public interface IServiceProviderRepository :IGlobalInterface
    {
        bool Insert(ServiceProviderModel model);
        ...
        bool Edit(string GUID, ServiceProviderModel model);
        bool Delete(string GUID);
        object GetServiceProvider(string ServiceProvider_guid);
    }
    public class ServiceProviderRepository : FormNumberGenerator, IServiceProviderRepository
    {
        public bool Edit(string GUID, ServiceProviderModel model)
        {
        ...
        {
            string sql = "UPDATE mswd.ServiceProvider SET status = 'Deleted' where mswd.ServiceProvider.ServiceProvider_guid = @ServiceProvider_guid";
        ...
        {
            string sql = @"SELECT mswd.service_provider.service_provider_guid, mswd.service_provider.form_trans_no, mswd.service_provider.application_date, mswd.service_provider.status,
                            ...
                            FROM mswd.service_provider
                            INNER JOIN general.person ON general.person.person_guid = mswd.service_provider.person_guid
                            WHERE mswd.service_provider.`status` = 'Active' ORDER BY mswd.service_provider.id DESC;";

            List<object> obj = (List<object>)QueryModule.DataSource<object>(sql);

            ExpandoObject form = Forms.getForm("service-provider");
        ...
        public object GetServiceProvider(string child_info_guid)
        {
            ...
            var obj = (ServiceProviderModel)QueryModule.DataObject<ServiceProviderModel>(sql, new { child_info_guid = child_info_guid });
            ...
            obj.family_details = (List<ServiceProviderDetails>)QueryModule.DataSource<ServiceProviderDetails>(sql);
        ...
        public object GetPersonAdd(string GUID)
        {
            ...
            var obj = (ServiceProviderModel)QueryModule.DataObject<ServiceProviderModel>(sql);
            ...
            //obj.details = (List<ServiceProviderDetails>)QueryModule.DataSource<ServiceProviderDetails>(sql);
        ...
        public bool Insert(ServiceProviderModel model)
        {
            if (QueryModule.connection.State == System.Data.ConnectionState.Open)
            {
                QueryModule.connection.Close();
